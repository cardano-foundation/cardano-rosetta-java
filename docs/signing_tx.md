 # what is signing_tx.ts
 
## Purpose:

signing_tx.ts is written to sign Rosetta Transaction CBOR code

The input is a Rosetta CBOR code that could be either the transaction and envelope (starting with 82...) or the Rosetta transaction payload (starting with a4 or a5...).

The output is the CBOR code including the signature
 
## Usage

- Declare the variable `rosetta_unsigned_tx` at line 14 and save the file

- Run the program (eg: `deno run --allow-all signing_data.ts`)

- The result is a signed CBOR

Here is an example

    $deno run --allow-all signing_data.ts 
    Unsigned Transaction:  
    Signed tx: 
