...
class AccountCoinsApiTest extends BaseSpringMvcSetup {
  ...
  void accountCoinsDifferentCoins_Test() {
    ...
    List<CoinTokens> metadata = accountCoinsResponse.getCoins().getFirst().getMetadata()
            .get(latestTxHashOnZeroSlot);
    assertEquals(1, metadata.size());
    String policyId = metadata.getFirst().getPolicyId();

    assertEquals(policyId, metadata.getFirst().getPolicyId());
    assertEquals(metadata.getFirst().getPolicyId(), metadata
            .getFirst().getTokens().getFirst().getCurrency().getMetadata().getPolicyId());

    assertEquals(policyId, metadata.getFirst().getTokens().getFirst().getCurrency().getMetadata().getPolicyId());
...
//  }
  ...
  void accountCoinsMultipleSpecifiedCurrencies_Test() {
    ...
    CoinTokens coinTokens = coinsMetadata.getFirst();

    assertEquals(coinsMetadata.getFirst().getPolicyId(), coinTokens.getPolicyId());
    ...
    assertEquals(TestConstants.ACCOUNT_BALANCE_MINTED_TOKENS_AMOUNT, coinTokens
            .getTokens().getFirst().getValue());
    assertEquals(coinTokens.getPolicyId(),
            coinTokens.getTokens().getFirst().getCurrency().getMetadata().getPolicyId());
    assertEquals(Constants.MULTI_ASSET_DECIMALS,

            coinTokens.getTokens().getFirst().getCurrency().getDecimals());
  }
