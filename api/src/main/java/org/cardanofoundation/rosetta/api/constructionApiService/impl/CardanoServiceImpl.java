...
//        return scope.manage(CardanoWasm.RewardAddress.from_address(wasmAddress));
  ...
  public String encodeExtraData(String transaction, TransactionExtraData extraData)
    ...
    extraOperations.stream().forEach(operation -> {
      ...
      Long index=operation.getOperationIdentifier().getNetworkIndex();
      ...
      if(!operationMetadataCheck){
        ...
        co.nstant.in.cbor.model.Map withdrawalAmount = getAmountMap(
            operationMetadataCheck ? null : operationMetadata.getWithdrawalAmount());
        co.nstant.in.cbor.model.Map depositAmount = getAmountMap(
            operationMetadataCheck ? null : operationMetadata.getDepositAmount());
        co.nstant.in.cbor.model.Map refundAmount = getAmountMap(
            operationMetadataCheck ? null : operationMetadata.getRefundAmount());
        ...
        co.nstant.in.cbor.model.Map stakingCredentialMap = getPublicKeymap(
            operationMetadataCheck ? null : operationMetadata.getStakingCredential());
        oMetadataMap.put(new UnicodeString("stakingCredential"), stakingCredentialMap);
        oMetadataMap.put(new UnicodeString("poolKeyHash"),
            new UnicodeString(operationMetadataCheck ? null : operationMetadata.getPoolKeyHash()));
        Double epoch = operationMetadataCheck ? null : operationMetadata.getEpoch();
        if (epoch != null) {
          oMetadataMap.put(new UnicodeString("epoch"),
              new DoublePrecisionFloat(epoch));
        }
        ...
        oMetadataMap.put(new UnicodeString("poolRegistrationCert"),
            new UnicodeString(
                operationMetadataCheck ? null : operationMetadata.getPoolRegistrationCert()));
        co.nstant.in.cbor.model.Map poolRegistrationParamsMap = new co.nstant.in.cbor.model.Map();
        PoolRegistrationParams poolRegistrationParams =
            operationMetadataCheck ? null : operationMetadata.getPoolRegistrationParams();
        poolRegistrationParamsMap.put(new UnicodeString("vrfKeyHash"),
            new UnicodeString(ObjectUtils.isEmpty(poolRegistrationParams) ? null
                : poolRegistrationParams.getVrfKeyHash()));
        poolRegistrationParamsMap.put(new UnicodeString("rewardAddress"), new UnicodeString(
            ObjectUtils.isEmpty(poolRegistrationParams) ? null
                : poolRegistrationParams.getRewardAddress()));
        poolRegistrationParamsMap.put(new UnicodeString("pledge"),
            new UnicodeString(ObjectUtils.isEmpty(poolRegistrationParams) ? null
                : poolRegistrationParams.getPledge()));
        poolRegistrationParamsMap.put(new UnicodeString("cost"),
            new UnicodeString(ObjectUtils.isEmpty(poolRegistrationParams) ? null
                : poolRegistrationParams.getCost()));
        Array poolOwnersArray = new Array();
        if (operationMetadata != null && !ObjectUtils.isEmpty(operationMetadata.getPoolRegistrationParams())) {
        ...
        PoolMargin poolMargin =
            ObjectUtils.isEmpty(poolRegistrationParams) ? null : poolRegistrationParams.getMargin();
        marginMap.put(new UnicodeString("numerator"), new UnicodeString(
            ObjectUtils.isEmpty(poolMargin) ? null : poolMargin.getNumerator()));
        marginMap.put(new UnicodeString("denominator"), new UnicodeString(
            ObjectUtils.isEmpty(poolMargin) ? null : poolMargin.getDenominator()));
        poolRegistrationParamsMap.put(new UnicodeString("margin"), marginMap);
        poolRegistrationParamsMap.put(new UnicodeString("marginPercentage"), new UnicodeString(
            ObjectUtils.isEmpty(poolRegistrationParams) ? null
                : poolRegistrationParams.getMarginPercentage()));
        co.nstant.in.cbor.model.Map poolMetadataMap = new co.nstant.in.cbor.model.Map();
        PoolMetadata poolMetadata = ObjectUtils.isEmpty(poolRegistrationParams) ? null
            : poolRegistrationParams.getPoolMetadata();
        marginMap.put(new UnicodeString("url"), new UnicodeString(
            ObjectUtils.isEmpty(poolMetadata) ? null : poolMetadata.getUrl()));
        marginMap.put(new UnicodeString("hash"), new UnicodeString(
            ObjectUtils.isEmpty(poolMetadata) ? null : poolMetadata.getHash()));
      ...
      if(!ObjectUtils.isEmpty(operation.getOperationIdentifier())) operationMap.put(new UnicodeString("operation_identifier"), operationIdentifierMap);
  ...
  public co.nstant.in.cbor.model.Map getPublicKeymap(PublicKey publicKey) {
    ...
    stakingCredentialMap.put(new UnicodeString("hexBytes"),
        new UnicodeString(ObjectUtils.isEmpty(publicKey)?null:publicKey.getHexBytes()));
    stakingCredentialMap.put(new UnicodeString("curveType"),
  ...
  public TransactionExtraData changeFromMaptoObject(co.nstant.in.cbor.model.Map map) {
    ...
    operationsListMap.stream().forEach(oDataItem -> {
      ...
      Double epoch = ((DoublePrecisionFloat) metadataMap.get(
          new UnicodeString("epoch"))).getValue();
