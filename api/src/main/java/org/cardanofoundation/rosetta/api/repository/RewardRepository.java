...
//                                        @Param("blockHash") String blockHash);
    ...
    Double findBalanceByAddressAndBlockSub1(@Param("stakeAddress") byte[] stakeAddress,
                                            @Param("blockHash") String blockHash);

    @Query("SELECT COALESCE(SUM(w.amount), 0) " +
            "FROM Withdrawal w " +
            "JOIN Tx tx on tx.id = w.tx.id " +
            ...
            "WHERE s.view = :stakeAddress ")
    Double findBalanceByAddressAndBlockSub2(@Param("stakeAddress") byte[] stakeAddress,
                                            @Param("blockHash") String blockHash);
