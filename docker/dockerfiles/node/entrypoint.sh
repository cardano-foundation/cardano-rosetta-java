...
cardano-node run --socket-path "$CARDANO_NODE_SOCKET_PATH" --port $CARDANO_NODE_PORT --database-path "$CARDANO_NODE_DB" --config /config/config.json --topology /config/topology.json &
CARDANO_NODE_PID=$!
sleep 2
